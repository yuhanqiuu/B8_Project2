...
void main (void)
{
	...
	unsigned int level[3],speaker_f;
	int volt_x;
	int volt_y;
	int count = 0;
	...
	{	
		...
		{	
			...
			level[count] = atoi(&buff[0]);

			// if(level[count] == 1) {
			// 	speaker_pulse();
			// }
			
			if(level[count] >= 0 && level[count] <= 6){ //check if the signal is in the range
				//printf("string=%s\r\n",buff);
				...
				if (count == 2) {
					...
					if(level[0]==6){
						...
						speaker_f = 200*((level[0]+level[1]+level[2])/3);
						buff[0] = (level[0]+level[1]+level[2])/3 + '0';
						buff[1] = '\0';
						LCDprint2(buff,1,11);
						// speaker beeps
						TR2=0; // Stop timer 2
